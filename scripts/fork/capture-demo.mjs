...
await page.evaluate(()=>window.forkRobot.curriculum('hazards'));await page.evaluate(()=>window.forkRobot.curriculum('weather'));await page.evaluate(()=>window.forkRobot.curriculum('crossing'));
...
while(Date.now()-start<118000){
 ...
 if(phase!==lastPhase){
  ...
  if(phase===6){await page.evaluate(()=>window.forkRobot.curriculum('weather'));await page.evaluate(()=>{window.forkRobot.select('after',20001);document.querySelector('.robot-title h1').innerHTML='The street gets harder.';document.querySelector('.robot-title p').innerHTML='Recorded wet-road physics · lower contact friction · one held-out weather failure remains';});}
  if(phase===7){await page.evaluate(()=>{document.querySelector('#demo-world').style.display='block';});await world.locator('#play').click();}
  if(phase===8)await page.evaluate(()=>{document.querySelector('#demo-world').style.display='none';const o=document.querySelector('#demo-overlay');o.style.display='block';o.innerHTML='<h1>Run the physics. In the same Chennai street.</h1><img src="/demo-media/native-mujoco.png"><p>Actual native MuJoCo renderer screenshot · live policy execution available · simplified native materials.</p>';});
  if(phase===9)await page.evaluate(()=>{document.querySelector('#demo-overlay').innerHTML='<img style="height:720px" src="/demo-media/architecture-simple.svg">';});
