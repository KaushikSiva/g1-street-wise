...
def mine(policy_path,difficulty,round_index,count=64):
    CentralAvenueStreetlife.difficulty=difficulty
    env=CentralAvenueStreetlife()
...
def main():
    ...
    args=p.parse_args()
    output=ROOT/'artifacts/fork/streetlife';output.mkdir(parents=True,exist_ok=True)
    ...
    while args.rounds==0 or round_index<=args.rounds:
        ...
        command=[sys.executable,str(ROOT/'scripts/fork/robot/train.py'),'--streetlife','--difficulty',str(difficulty),'--seed-offset',str(5000000+round_index*100000),'--steps',str(args.steps),'--eval-every','8192','--resume',str(checkpoint),'--output',str(folder),'--device',args.device,'--learning-rate','0.0001','--entropy','0.01']
        ...
        failures,rows=mine(checkpoint,difficulty,round_index)
