...
def decorate(environment='crossing'):
    if environment=='emergencies':
        from emergencies import emergency_scene
        source=emergency_scene()
    elif environment=='streetlife':
...
def prepare():
    ...
    for i,group in enumerate(data['groups']):
        ...
        for part,start in enumerate(range(0,len(faces),150000)):
            ...
            meshes.append({'file':file,'name':group.get('name',''),'sourceObjects':group.get('sourceObjects',[]),'sourceGroup':i,'sourceFaceStart':start,'sourceFaceCount':len(source_faces),'discardedDegenerateTriangles':int((~valid).sum()),'color':group['color'],'texture':lookup.get(group.get('texture')),'roughness':group.get('roughness',.8),'triangles':len(f),'visualBacking':bool(planar)})
    ...
    manifest['sceneMetadata']=data.get('sceneMetadata',[])
    manifest['exportedAt']=data.get('exportedAt')
    manifest['sourceExportSha256']=hashlib.sha256((ROOT/'.fork-runs/robot/chennai-native.json').read_bytes()).hexdigest()
