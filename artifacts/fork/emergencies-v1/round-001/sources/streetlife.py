...
def streetlife_scene():
    ...
    for i in range(3):
        shelter=ET.SubElement(world,'body',name=f'shelter_{i}',mocap='true',pos='0 100 0')
        ET.SubElement(shelter,'geom',name=f'shelter_roof_{i}',type='box',pos='0 0 2.65',size='.7 .65 .06',contype='0',conaffinity='0',rgba='.28 .45 .39 1')
...
class CentralAvenueStreetlife(CentralAvenueWeather):
    ...
    def reset(self,*,seed=None,options=None):
        ...
        self.robot.data.mocap_pos[3:5]=[[0,100,.85],[0,100,.85]]
        rng=np.random.default_rng(seed+90000)
        self.seek_shelter=self.condition=='rain'
        candidates=[[-.6,1.95],[1.7,-1.95],[3.3,1.95]]
        self.shelters=[np.array(p,dtype=float) for i,p in enumerate(candidates) if i!=(seed%4)]
        if len(self.shelters)<2:self.shelters=[np.array(p,dtype=float) for p in candidates]
        for i in range(3):self.robot.data.mocap_pos[5+i]=[*self.shelters[i],0] if i<len(self.shelters) else [0,100,0]
        self.goal=min(self.shelters,key=lambda p:np.linalg.norm(self.robot.data.qpos[:2]-p)).copy() if self.seek_shelter else np.array([3.2,0.])
    ...
    def _observe(self):
        ...
        if self.seek_shelter and self.shelters:self.goal=min(self.shelters,key=lambda p:np.linalg.norm(d.qpos[:2]-p)).copy()
        extra.extend([(self.goal[0]-d.qpos[0])/4,(self.goal[1]-d.qpos[1])/3,float(self.seek_shelter)])
        for i in range(3):
            extra.extend([(self.shelters[i][0]-d.qpos[0])/4,(self.shelters[i][1]-d.qpos[1])/3,1.]) if i<len(self.shelters) else extra.extend([0.,0.,0.])
    ...
    def goal_reached(self):
        ...
        return bool(any(np.linalg.norm(p-goal)<.45 for goal in self.shelters)) if self.seek_shelter else bool(p[0]>self.goal[0])
    ...
    def extra_reward(self,old_position):
        ...
        return 3*(min(float(np.linalg.norm(old_position-goal)) for goal in self.shelters)-min(float(np.linalg.norm(current-goal)) for goal in self.shelters))-2*(current[0]-old_position[0])-.035
    ...
    def episode_record(self):
        return {**super().episode_record(),'goal':self.goal.tolist(),'shelters':[p.tolist() for p in self.shelters],'seek_shelter':self.seek_shelter,'people':[dict(a) for a in self.people],
